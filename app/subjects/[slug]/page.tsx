...
type ChapterListEditModalProps = {
  isOpen: boolean;
...
function ChapterListEditModal({
  isOpen,
  chapters,
  isSubmitting,
  onClose,
  onSubmit,
}: ChapterListEditModalProps) {
  ...
  useEffect(() => {
    ...
    setValue(chapters.map((chapter) => chapter.name).join("\n"));
    setError(null);
  }, [chapters, isOpen]);
  ...
  const parsedNames = value
    ...
    .map(stripListMarker)
    .filter(Boolean);
  const existingNames = chapters.map((chapter) => chapter.name);
  const hasChanges =
    parsedNames.length !== existingNames.length ||
    parsedNames.some((name, index) => name !== existingNames[index]);
  const addedCount = Math.max(0, parsedNames.length - chapters.length);
  const deletedCount = Math.max(0, chapters.length - parsedNames.length);
  const matchedCount = Math.min(parsedNames.length, chapters.length);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (parsedNames.length === 0) {
      setError("কমপক্ষে একটি অধ্যায়ের নাম রাখুন।");
      return;
    }
    ...
    try {
      ...
      console.error("Failed to update chapter list:", submitError);
      setError("অধ্যায় লিস্ট আপডেট করা যায়নি। আবার চেষ্টা করুন।");
    }
  };
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-border-subtle p-6 dark:border-white/10">
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-gray-500 dark:text-neutral-400">
              প্রতি লাইনে একটি অধ্যায় রাখুন। নতুন লাইন যোগ করলে নতুন অধ্যায়
              হবে, লাইন মুছলে অধ্যায় মুছে যাবে।
            </p>
          </div>
          <button
            type="button"
            ...
            aria-label="অধ্যায় লিস্ট এডিট বন্ধ করুন"
          >
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 p-6">
          <textarea
            ...
            placeholder="প্রতি লাইনে একটি অধ্যায়ের নাম লিখুন"
            disabled={isSubmitting}
            spellCheck={false}
          />
          ...
          <div className="flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-gray-500 dark:text-neutral-400">
              সেভের পর {parsedNames.length}টি অধ্যায় থাকবে
            </p>
            <p className="text-sm text-gray-500 dark:text-neutral-400">
...
export default function SubjectPage() {
  ...
  const syncChapterList = useMutation(api.mutations.syncChapterList);
  ...
  if (data === null) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24">
        <span className="material-symbols-outlined text-5xl text-gray-300 dark:text-neutral-700">
          error_outline
        </span>
  ...
  const nextTermChapters = chapters
    ...
    .sort(
      (left, right) =>
        (left.nextTermOrder ?? left.order) -
          (right.nextTermOrder ?? right.order) ||
  ...
  const handleListUpdate = async (names: string[]) => {
    ...
    try {
      const result = await syncChapterList({ subjectId: subject._id, names });
      clearSelection();
  ...
  return (
    <div className="w-full">
      <SubjectHeader name={subject.name} progressPercentage={progressPercentage} />

      <div className="mb-8 flex flex-col gap-3 md:-mt-4 md:flex-row md:justify-end">
        <button
          type="button"
          onClick={() => setIsListEditOpen(true)}
          disabled={!hasChapters}
          className="flex w-full items-center justify-center gap-2 rounded-full border border-border-medium bg-pure-white px-5 py-3 font-label-uppercase text-label-uppercase text-on-surface shadow-sm transition-colors hover:border-border-strong hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:py-2.5 dark:border-white/10 dark:bg-white/[0.04] dark:text-neutral-100 dark:hover:border-white/20 dark:hover:bg-white/[0.08]"
        >
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-full bg-on-surface px-6 py-3 font-label-uppercase text-label-uppercase text-pure-white shadow-sm transition-all hover:bg-brand-green hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-green focus-visible:ring-offset-2 sm:w-auto sm:py-2.5 dark:bg-neutral-100 dark:text-neutral-950 dark:hover:bg-brand-green dark:focus-visible:ring-offset-neutral-950"
        >
